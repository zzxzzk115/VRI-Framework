-- xr_triangle: validation example for the vrf::xr module - probe-first device
-- creation, XrSession multiview stereo on a runtime, SimStereoRig side-by-side
-- desktop fallback (--sim). Shader handling mirrors the other examples.

if has_config("vrf_cook_shaders") then
    rule("vrf.cook_xr_triangle_shaders")
...
rule("vrf.embed_xr_triangle_shaders")
    on_config(function (target)
        target:add("includedirs", path.join(target:autogendir(), "shaders"))
    end)
    before_build(function (target)
        local src    = path.join(target:scriptdir(), "shaders", "xr_triangle.vshlib")
        ...
        io.writefile(path.join(gendir, "xr_triangle.vshlib.h"), table.concat(parts))
    end)
rule_end()

target("vrf_example_xr_triangle")
    set_kind("binary")
    set_languages("cxx23")
    ...
    if has_config("vrf_cook_shaders") then
        add_rules("vrf.cook_xr_triangle_shaders")
    end
    add_rules("vrf.embed_xr_triangle_shaders")

    add_files("main.cpp")

    set_targetdir("$(builddir)/$(plat)/$(arch)/$(mode)/vrf_example_xr_triangle")
