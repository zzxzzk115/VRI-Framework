-- gaussian_rt: inline ray tracing (RayQuery) validation. Headless: a
...
if has_config("vrf_cook_shaders") then
    rule("vrf.cook_gaussian_rt_shaders")
        before_build(function (target)
            ...
            os.vrunv(vshaderc, {"build", "--shader_root", shaderRoot, "-o", path.join(shaderRoot, "gaussian_rt.vshlib"), "--no-wgsl"})
            cprint("${green}[cook]${clear} regenerated shaders/gaussian_rt.vshlib")
        end)
    rule_end()
end

rule("vrf.embed_gaussian_rt_shaders")
    on_config(function (target)
        target:add("includedirs", path.join(target:autogendir(), "shaders"))
    end)
    before_build(function (target)
        local src    = path.join(target:scriptdir(), "shaders", "gaussian_rt.vshlib")
        ...
        io.writefile(path.join(gendir, "gaussian_rt.vshlib.h"), table.concat(parts))
    end)
rule_end()

target("vrf_example_gaussian_rt")
    set_kind("binary")
    set_languages("cxx23")
    add_deps("vrf")
    if has_config("vrf_cook_shaders") then
        add_rules("vrf.cook_gaussian_rt_shaders")
    end
    add_rules("vrf.embed_gaussian_rt_shaders")
    add_files("main.cpp")
    set_targetdir("$(builddir)/$(plat)/$(arch)/$(mode)/vrf_example_gaussian_rt")
