 ...
 * access/layout/stage state and a cache of the views (DescriptorHandle) the
 * framegraph needs: attachment views per subresource, one sampled view per
 * aspect, and storage views per mip.
 *
 * A Texture either owns its TextureHandle (transient-pool path) or borrows an
...
namespace vrf::fg
{
    ...
    public:
        ...
        Borrow(RenderDevice&, TextureHandle*, const Desc&, const VriAccessLayoutStage& state);

        [[nodiscard]] TextureHandle* Handle() const noexcept { return m_texture; }
        ...
        [[nodiscard]] DescriptorHandle* AttachmentView(ImageAspect,
                                                       std::optional<uint32_t> layer = std::nullopt,
                                                       std::optional<CubeFace> face  = std::nullopt);
        // Whole-resource sampled view of one aspect.
        [[nodiscard]] DescriptorHandle* SampledView(ImageAspect = ImageAspect::Color);
        // Storage (UAV) view of one mip.
        [[nodiscard]] DescriptorHandle* StorageView(uint32_t mipLevel = 0);
    ...
    private:
        Texture(RenderDevice& device, TextureHandle* texture, const Desc& desc, bool owned) noexcept;

        void DestroyViews() noexcept;
        void Reset() noexcept;

        [[nodiscard]] DescriptorHandle* GetOrCreateView(uint64_t key, const VriTextureViewDesc&);
        ...
        std::unordered_map<uint64_t, DescriptorHandle*> m_views;
    };
} // namespace vrf::fg
